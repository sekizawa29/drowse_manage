...
export function ImportSales({ selectedMonth, onImportComplete }: ImportSalesProps) {
      ...
      try {
        ...
        Papa.parse(csvText, {
          ...
          complete: (results) => {
            ...
            // 必要なヘッダーの確認
            const requiredHeaders = ["日付", "製品名", "カテゴリ", "数量", "金額"]
            ...
            data.forEach((row: any) => {
              try {
                ...
                if (
                  ...
                  // 数値の解析
                  const quantity = Number.parseInt(row["数量"], 10)
                  const amount = Number.parseInt(row["金額"], 10)

                  if (isNaN(quantity) || isNaN(amount)) {
                    throw new Error("数量または金額が無効です。")
                  }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>売上データのインポート</DialogTitle>
          <DialogDescription>
            CSVファイルから売上データをインポートします。
            <br />
        ...
        <div className="space-y-4 py-4">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-xs text-muted-foreground">
              ヘッダー行に「日付,製品名,カテゴリ,数量,金額」を含むCSVファイルを選択してください。
            </p>
          </div>
        </div>
